#!/bin/bash
# Experiment A: re-run annotate.py on 4 vertebrates_test species with
# the HMM start-state distribution restricted to {IR, START} via
# --restrict-start-to-ir-start. Same epoch_11 weights as the baseline.
#
# Output:  ${RESULTS_DIR}/<sp>/annotate_${WEIGHTS_TAG}_ir_start/orfs.gtf
#
#SBATCH --job-name=annot_vt_ir
...
#SBATCH --array=0-3
#SBATCH --output=/projects/AI-GUSTUS/tiberius_orf_finder/logs/annot_vt_ir_%A_%a.out
#SBATCH --error=/projects/AI-GUSTUS/tiberius_orf_finder/logs/annot_vt_ir_%A_%a.err
...
OUTDIR=${RESULTS_DIR}/${species}/annotate_${WEIGHTS_TAG}_ir_start
