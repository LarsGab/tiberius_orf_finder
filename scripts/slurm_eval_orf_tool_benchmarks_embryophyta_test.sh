#!/bin/bash
# gffcompare Tiberius (run001 e39) + TransDecoder1 + TransDecoder2 +
# TransDecoder2(--precise) + GeneMarkS-T against the reference for each
# embryophyta test species. Produces accuracy_table.tsv + PDF plot.
#
# Run after td1/td2/td2p/gmst array jobs complete (or submit immediately
# — "[skip]" lines are printed for any missing inputs).
#
#SBATCH --job-name=eval_orf_emb
#SBATCH --partition=snowball,pinky,batch
#SBATCH --cpus-per-task=2
#SBATCH --mem=8G
#SBATCH --time=01:00:00
#SBATCH --output=/projects/AI-GUSTUS/tiberius_orf_finder/logs/eval_orf_emb_%j.out
#SBATCH --error=/projects/AI-GUSTUS/tiberius_orf_finder/logs/eval_orf_emb_%j.err
