#!/bin/bash
# Experiment C2: same as flank20 but with --flank-clip.
# Instead of dropping ORFs whose START falls in the genomic flank, clip
# them to start at the next in-frame codon boundary at or after the
# transcript 5' end (5'-truncated CDS). Lets us recover ORFs where the
# decoder found an attractive upstream ATG.
#
# Output:  ${RESULTS_DIR}/<sp>/annotate_${WEIGHTS_TAG}_flank20_clip/orfs.gtf
#
#SBATCH --job-name=annot_vt_flank20_clip
...
#SBATCH --array=0-3
#SBATCH --output=/projects/AI-GUSTUS/tiberius_orf_finder/logs/annot_vt_flank20_clip_%A_%a.out
#SBATCH --error=/projects/AI-GUSTUS/tiberius_orf_finder/logs/annot_vt_flank20_clip_%A_%a.err
...
OUTDIR=${RESULTS_DIR}/${species}/annotate_${WEIGHTS_TAG}_flank20_clip
