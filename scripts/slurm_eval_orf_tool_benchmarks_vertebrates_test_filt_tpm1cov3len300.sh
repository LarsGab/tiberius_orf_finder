#!/bin/bash
# gffcompare Tiberius (subseq-filtered) + TransDecoder1 + TransDecoder2
# + TransDecoder2(--precise) + GeneMarkS-T against the reference for
# the 4 vertebrates_test species with tpm1cov3len300-filtered StringTie
# input. Aggregates into a single accuracy_table.
#
# Tiberius baseline for this variant: run006/epoch_74 on the filtered
# stringtie, THEN subsequence-collapse postprocess. Only Tiberius gets
# the post-filter — TD1/TD2/GMST use the filtered stringtie only.
#
# Output:
#   ${RESULTS_DIR}/benchmark_orf_tools_filt_tpm1cov3len300/
#     <sp>/gffcompare/<tool>.stats
#     accuracy_table.tsv
#
#SBATCH --job-name=eval_orf_f_vt
#SBATCH --partition=snowball,pinky,batch
#SBATCH --cpus-per-task=2
#SBATCH --mem=8G
#SBATCH --time=01:00:00
#SBATCH --output=/projects/AI-GUSTUS/tiberius_orf_finder/logs/eval_orf_f_vt_%j.out
#SBATCH --error=/projects/AI-GUSTUS/tiberius_orf_finder/logs/eval_orf_f_vt_%j.err

set -euo pipefail

PROJDIR=/projects/AI-GUSTUS/tiberius_orf_finder
RESULTS_DIR=${PROJDIR}/results/vertebrates_test
TIB_TAG=${TIB_TAG:-annotate_epoch_74_filt_tpm1cov3len300}
...
for sp in "${SPECIES[@]}"; do
    ...
    declare -A GTF
    # Tiberius uses the subseq-collapsed output (postprocess-filtered).
    GTF[tiberius]=${RESULTS_DIR}/${sp}/${TIB_TAG}/orfs.filtered.gtf
