#!/bin/bash
# TD2 (TransDecoder2) DEFAULT on the tpm1cov3len300-filtered StringTie
# assemblies. Sibling of slurm_benchmark_transdecoder2_vertebrates_test.sh
# but with the filtered StringTie as input. Runs on CPU with
# CUDA_VISIBLE_DEVICES="" — PSAURON's cuDNN version doesn't match the
# vision-partition GPUs on brain (documented in the previous run).
#
# Output layout:
#   ${RESULTS_DIR}/<sp>/benchmark_orf_tools/transdecoder2_filt_tpm1cov3len300/
#     transcripts.fa                    (link to shared filtered fasta)
#     transcripts.fa.TD2.gff3
#     orfs.gtf
#     orfs.gtf.stats.txt
#     td2_workdir/                       (LongOrfs intermediates)
#
#SBATCH --job-name=td2_vt_f
...
# `td2` is a dedicated CPU-only env (pytorch cpuonly + td2 + gffread +
# numpy). Isolated from the `orffinder` env, which still ships the
# CUDA-flavoured pytorch that fights with TF's NCCL.
...
# Belt-and-braces: even with the CPU-only pytorch build, some psauron
# releases still try to probe CUDA at import time.
export CUDA_VISIBLE_DEVICES=""

cd "${OUTDIR}"
echo "[$(date -Iseconds)] species=${species} variant=default filter=${FILT_TAG}"
...
# TD2.LongOrfs refuses to overwrite -O dir; nuke stale first.
